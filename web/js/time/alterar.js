...
document.addEventListener("DOMContentLoaded", function () {
    ...
    firebase.storage().ref().child("time/" + id).getDownloadURL().then(url => {
        fotopreview.src = url;
    });
...
function alterarDados() {
   var nome = document.getElementById("nome").value;
    ...
    firebase.database().ref("time").child(id).update({
        ...
        if (uploadfoto.value != "") {
            firebase.storage().ref('time').child(id).delete().then(function () {
                ...
                reader.onload = function () {
                    ...
                    firebase.storage().ref('time/').child(id).putString(base64, 'base64', {contentType: 'image/png'}).then(snapshot => {
                        console.log('snapshot', snapshot);
                        alert("Dados alterados");
                        window.location = "../jsp/meus-times.jsp";
