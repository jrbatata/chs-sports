...
function informacoes(id) {
    ...
    ref.once('value').then(snapshot => {
        snapshot.forEach(value => {
            adicionaTimesATela(value.val(), id);
        });
        if (!existeTime) {
            naoTem();
        }
...
function deletar(idCampeonato) {
    firebase.database().ref('time').child(idCampeonato).remove().then(() => {

    });

    firebase.storage().ref('time').child(idCampeonato).delete().then(function () {
...
function adicionaTimesATela(informacao, id) {
    var idTime = informacao.timeId;
    ...
    if (informacao.idGerente == id) {
        ...
        a.setAttribute("href", "campeonato.jsp");
        ...
        editar.setAttribute("onclick", "passaValor('" + idTime + "','alterar/time.jsp')");
        ;
        ...
        excluir.setAttribute("data-whatever", informacao.timeId);
        ...
        ver.setAttribute("onclick", "passaValor('" + idTime + "','jsp/time.jsp')");
        ...
        var fotopreview = document.getElementById(idTime);
        firebase.storage().ref().child("time/" + idTime).getDownloadURL().then(url => {
            fotopreview.src = url;
        });
        existeTime = true;
...
{
    window.location = "../" + url + "?time=" + valor;
}
...
function naoTem() {
    ...
    h1.innerText = "Você não criou nenhum TIME ainda";

    var a = document.createElement("a");
    a.setAttribute("href", "../cadastro/time.jsp");

    var h2 = document.createElement("h2");
    h2.setAttribute("style", "font-size: 20px");
    h2.innerText = "Clique aqui para criar";
